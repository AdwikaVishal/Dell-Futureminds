...
def test_inject_scenario(name: str, payload: dict):
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            print(f"Top 3 after injection:")
            for i, t in enumerate(top3[:3]):
                score_breakdown = t.get("score_breakdown", {})
                print(f"  {i+1}. [{t.get('priority', 'N/A')}] {t.get('title', '')[:60]}")
                print(f"      Score: {t.get('score', 0)} | Breakdown: {json.dumps(score_breakdown)}")
...
def main():
    ...
    test_inject_scenario("P1 Production Outage", {
    ...
    test_inject_scenario("Critical Security Vulnerability", {
    ...
    test_inject_scenario("Low Priority - Update README", {
    ...
    test_inject_scenario("VP Escalation - Customer Downtime", {
