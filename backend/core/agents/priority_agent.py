...
class PriorityAgent(BaseAgent):
    ...
    async def process(self, context: dict[str, Any]) -> dict[str, Any]:
        ...
        if delegated_tasks:
            logger.info("Filtered out %d delegated tasks (owned by others)", len(delegated_tasks))

        try:
            ranked_tasks = await prioritize(my_tasks)
            logger.info("Prioritized %d tasks (%d delegated excluded)", len(ranked_tasks), len(delegated_tasks))
        ...
        if leverage:
            logger.info("Highest leverage task: %s (score: %.1f)", leverage[0]["task_id"], leverage[0]["leverage_score"])
    ...
    async def reflect(self, context: dict[str, Any]) -> dict[str, Any]:
        ...
        reflection["observations"] = [f"Scored {len(ranked)} tasks using deterministic 7-factor formula"]

        if ranked:
            top = ranked[0]
            reflection["observations"].append(f"Top task: {top.id} ({top.title[:40]}) score={top.score}")
            if top.score_breakdown:
                biggest = max(top.score_breakdown, key=top.score_breakdown.get)
                reflection["decisions"] = [f"Biggest score driver: {biggest}={top.score_breakdown[biggest]}"]
