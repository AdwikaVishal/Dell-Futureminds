    ...
    async def run_pipeline(self, initial_context: dict[str, Any] | None = None) -> dict[str, Any]:
        ...
        for agent in self.agents:
            ...
            try:
                ...
                if not verification.get("verified", True):
                    logger.warning("Agent %s verification failed: %s", agent_name, verification)
                ...
                memory_system.record_agent_memory(agent_name, "last_pipeline_result",
        ...
        if ranked_tasks:
            try:
                ...
                unblocking_recs = DependencyAnalyzer.get_unblocking_recommendations(ranked_tasks)
                ...
                time_blocked = CalendarPlanner.generate_time_blocked_plan(ranked_tasks[:min(6, len(ranked_tasks))])
        ...
        logger.info("=== Pipeline %s complete (%.0fms) ===", pipeline_id,
