    ...
    <div style={{ display: "flex", flexDirection: "column", height: "100%", overflow: "hidden" }}>
      ...
      <main style={{ flex: 1, overflow: "auto", padding: "28px" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
          {tasks.map((task: any) => (
            <div key={task.id}>
              <div style={{ cursor: "pointer", border: "1px solid #D9D9D9", padding: "13px 16px", background: "var(--bg-elevated)" }}>
                <div onClick={() => setExpanded(expanded === task.id ? "" : task.id)} style={{ display: "flex", alignItems: "center", gap: 12 }}>
                  ...
                  <span style={{ color: "var(--text-secondary)", fontSize: 11, fontFamily: "'IBM Plex Mono', monospace", minWidth: 72 }}>{task.id}</span>
                  <span style={{ color: "var(--text-primary)", fontSize: 13, flex: 1 }}>{task.title}</span>
                  <SourceBadge source={task.source || task.source_type} />
                  <PriorityPill level={(task.priority ?? "P3") as any} />
                  <span style={{ color: "var(--text-secondary)", fontSize: 12, minWidth: 110, fontFamily: "'IBM Plex Mono', monospace" }}>{task.deadline || ""}</span>
                  <StatusPill status={task.status || "open"} />
                  {(task.merged_sources?.length > 0 || task.merged_from?.length > 0) && (
                    <span style={{ display: "flex", alignItems: "center", gap: 4, color: "var(--blue-primary)", fontSize: 11, padding: "2px 8px", border: "1px solid var(--blue-primary)", whiteSpace: "nowrap", fontFamily: "'IBM Plex Mono', monospace" }}>
