  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%", overflow: "hidden" }}>
      ...
      <main style={{ flex: 1, overflow: "auto", padding: "28px" }}>
        ...
        <div style={{ border: "1px solid #D9D9D9", padding: "10px 16px", background: "var(--bg-elevated)", display: "flex", alignItems: "center", gap: 8, marginBottom: 20 }}>
          <SparkleIcon size={14} />
          <span style={{ color: "var(--text-secondary)", fontSize: 13 }}>
            Live data from {displaySources.filter(s => s.status === "Synced").length}/{displaySources.length} connected sources
            {extractions.length > 0 && (
              <span style={{ marginLeft: 4 }}>
                &middot; <strong style={{ color: "var(--blue-primary)" }}>{extractions.length}</strong> hidden action items found
              </span>
            )}
          </span>
        </div>
          ...
          <>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 12, marginBottom: 28 }}>
              {displaySources.map((src) => (
                <div key={src.name} style={{ border: "1px solid #D9D9D9", padding: "16px", background: "var(--bg-elevated)" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 12 }}>
                    <div style={{ width: 36, height: 36, border: "1px solid #D9D9D9", display: "flex", alignItems: "center", justifyContent: "center" }}>
                      <span style={{ fontSize: 10, fontWeight: 700, color: "var(--text-secondary)", fontFamily: "'IBM Plex Mono', monospace" }}>{src.name.slice(0, 2).toUpperCase()}</span>
                    </div>
                    <div style={{ display: "flex", alignItems: "center", gap: 5 }}>
                      <span style={{ width: 7, height: 7, borderRadius: "50%", background: src.status === "Synced" ? "#16A34A" : "#D97706", display: "inline-block" }} />
                      <span style={{ color: "var(--text-secondary)", fontSize: 11, fontFamily: "'IBM Plex Mono', monospace" }}>{src.status}</span>
                    </div>
                  </div>
                  <div style={{ color: "var(--text-primary)", fontSize: 13, fontWeight: 500, marginBottom: 4 }}>{src.name}</div>
                  <div style={{ color: "var(--text-secondary)", fontSize: 12, marginBottom: 4 }}>Live data via API</div>
                  <div style={{ color: "var(--text-secondary)", fontSize: 11, fontFamily: "'IBM Plex Mono', monospace" }}>
                    Status: {src.status}
            ...
            <div style={{ color: "var(--text-secondary)", fontSize: 12, fontWeight: 500, letterSpacing: "0.06em", textTransform: "uppercase", marginBottom: 12, fontFamily: "'IBM Plex Mono', monospace" }}>
            ...
            {extractions.length === 0 && !loading && (
              <div style={{ border: "1px solid #D9D9D9", padding: 40, background: "var(--bg-elevated)", textAlign: "center" }}>
