...
export function Screen0({ onStart }: { onStart: () => void }) {
  return (
    <div style={{
      ...
      <div style={{ borderBottom: "1px solid var(--border-default)", padding: "16px 40px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <div style={{ width: 32, height: 32, borderRadius: 10, background: "var(--bg-sidebar)", display: "flex", alignItems: "center", justifyContent: "center" }}>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          {["Features", "How it Works", "Demo"].map((l) => (
            <button key={l} style={{ background: "none", border: "none", padding: "8px 16px", fontSize: 13, color: "var(--text-secondary)", cursor: "pointer" }}>{l}</button>
          ))}
      ...
      <div style={{ maxWidth: 1100, margin: "0 auto", padding: "60px 40px" }}>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 60, alignItems: "center", marginBottom: 80 }}>
          <div>
            ...
            <div style={{ display: "flex", gap: 12 }}>
              ...
              <button style={{
                background: "none", border: "1px solid var(--border-default)", borderRadius: 14,
                padding: "14px 32px", fontSize: 14, fontWeight: 500, cursor: "pointer", color: "var(--text-primary)",
              }}>
                Watch Demo
              </button>
            </div>
          ...
          <div style={{
            ...
            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 16 }}>
              ...
              <span style={{ color: "var(--text-muted)", fontSize: 12 }}>14 tasks · 3 urgent</span>
            </div>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 12 }}>
              {["#F7C5E6", "#F5D66E", "#BFD78D", "#C9D8FF"].map((c, i) => (
                <div key={i} style={{ background: c, borderRadius: 12, padding: "12px 14px" }}>
                  <div style={{ fontSize: 11, color: "var(--text-secondary)", marginBottom: 4 }}>Card {i + 1}</div>
            ...
            <div style={{ background: "#F7C5E6", borderRadius: 14, padding: "14px 16px" }}>
              ...
              <div style={{ fontSize: 11, color: "var(--text-secondary)" }}>P1 · Due in 3h · Blocks: 2 teammates</div>
            </div>
          </div>
        </div>
        ...
        <div style={{ marginBottom: 80 }}>
          ...
          <p style={{ textAlign: "center", color: "var(--text-secondary)", fontSize: 15, margin: "0 0 32px" }}>Engineers lose 4+ hours/week just figuring out what to work on.</p>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 16 }}>
            {PROBLEMS.map((p) => (
              <div key={p.label} style={{ background: "var(--bg-elevated)", borderRadius: 18, border: "1px solid var(--border-default)", padding: 24 }}>
                <div style={{ fontSize: 28, marginBottom: 12 }}>{p.emoji}</div>
                <div style={{ fontWeight: 600, fontSize: 15, marginBottom: 6 }}>{p.label}</div>
                <div style={{ color: "var(--text-secondary)", fontSize: 13 }}>{p.desc}</div>
              </div>
            ))}
          </div>
        </div>
        ...
        <div style={{ marginBottom: 80 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 16 }}>
            {FEATURES.map((f) => {
              const Icon = f.icon;
              return (
                <div key={f.label} style={{ background: "var(--bg-elevated)", borderRadius: 18, border: "1px solid var(--border-default)", padding: 24 }}>
          ...
          <div style={{ display: "flex", gap: 16, justifyContent: "center" }}>
            {[
              ...
              <div key={s.step} style={{ textAlign: "center", flex: 1 }}>
                <div style={{
                  width: 48, height: 48, borderRadius: 14, background: "var(--bg-sidebar)",
      ...
      <div style={{ borderTop: "1px solid var(--border-default)", padding: "24px 40px", textAlign: "center", color: "var(--text-muted)", fontSize: 13 }}>
