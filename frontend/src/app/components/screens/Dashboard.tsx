  ...
  return (
    <Card variant={variant as any} shadow>
      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 10 }}>
        <span style={{ fontSize: 11, color: "var(--text-secondary)", fontWeight: 500, textTransform: "uppercase", letterSpacing: "0.04em", fontFamily: "'IBM Plex Mono', monospace" }}>{label}</span>
        {icon}
      </div>
...
export function Dashboard() {
  ...
  const rest = useMemo(() => {
    ...
    return [...(plan.do_next ?? []), ...(plan.deferred ?? []), ...(plan.blocked ?? [])].filter(
      (task) => { if (seen.has(task.id)) return false; seen.add(task.id); return true; }
    );
  }, [plan]);
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 14 }}>
        <KPI icon={<BarChart3 size={16} color="var(--text-secondary)" />} label="Total Tasks" value={String(allRanked.length)} sub="Active tracked" variant="blue" />
        <KPI icon={<AlertTriangle size={16} color="var(--text-secondary)" />} label="Top Priority Score" value={top3[0] ? String(top3[0].score) : "--"} sub={top3[0]?.title?.slice(0, 24) || "No tasks"} variant="pink" />
        <KPI icon={<Zap size={16} color="var(--text-secondary)" />} label="Hidden Tasks" value={String(deferredTasks.length)} sub="AI-extracted" variant="green" />
        <KPI icon={<Clock size={16} color="var(--text-secondary)" />} label="Focus Hours" value={focusHours} sub={`${timeBlocks.length} blocks scheduled`} variant="yellow" />
      </div>

      {narrativeAlert && !narrativeDismissed && (
        <div style={{ display: "flex", alignItems: "flex-start", gap: 8, padding: "10px 14px", background: "var(--bg-primary)", border: "1px solid var(--blue-primary)", borderRadius: 12, fontSize: 12, color: "var(--text-primary)" }}>
          <MessageSquare size={14} className="shrink-0 mt-0.5" style={{ color: "var(--blue-primary)" }} />
          <div style={{ flex: 1 }}>{narrativeAlert}</div>
          <button onClick={() => setNarrativeDismissed(true)} style={{ background: "none", border: "none", cursor: "pointer", color: "var(--text-secondary)", fontSize: 14, padding: 0 }}>×</button>
        </div>
      )}
      ...
      <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
        ...
        <button onClick={handleInjectP1} disabled={injecting}
          style={{ background: "none", border: "1px solid var(--border-default)", padding: "10px 20px", borderRadius: 12, fontSize: 12, cursor: injecting ? "not-allowed" : "pointer", color: "var(--text-secondary)", fontFamily: "'IBM Plex Mono', monospace" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          <div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {top3.length === 0 && (
                <Card><span style={{ color: "var(--text-secondary)", fontSize: 13 }}>No priorities ranked yet. Generate a plan.</span></Card>
              )}
                ...
                <Card key={task.id} variant={i === 0 ? "pink" : i === 1 ? "yellow" : "blue"} shadow>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 10 }}>
                    <div style={{ flex: 1 }}>
                      ...
                      <div style={{ color: "var(--text-primary)", fontSize: 14, fontWeight: 500, lineHeight: 1.4 }}>{task.title}</div>
                    </div>
                  ...
                  {task.score_breakdown && Object.keys(task.score_breakdown).length > 0 && (
                    <div style={{ display: "flex", gap: 8, marginTop: 6, flexWrap: "wrap" }}>
                      {Object.entries(task.score_breakdown).map(([k, v]) => (
                        <span key={k} style={{ fontSize: 10, color: "var(--text-secondary)", background: "rgba(0,0,0,0.04)", padding: "2px 6px", borderRadius: 6, fontFamily: "'IBM Plex Mono', monospace" }}>
                          {k.replace(/_/g, " ")}: {v}
                        </span>
                      ))}
                    </div>
          ...
          <div>
            ...
            {rest.length === 0 && !loading && (
              <Card><span style={{ color: "var(--text-secondary)", fontSize: 13 }}>Run the pipeline to see tasks here.</span></Card>
            )}
            {rest.map((task: any) => (
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          <Card variant="orange" shadow>
            ...
            {alerts.length > 0 ? alerts.slice(0, 4).map((a: any, i: number) => (
              <div key={i} style={{ padding: "8px 0", borderBottom: i < 3 ? "1px solid rgba(0,0,0,0.06)" : "none", fontSize: 12, color: "var(--text-secondary)", display: "flex", gap: 6 }}>
                <span style={{ width: 6, height: 6, borderRadius: "50%", background: a.severity === "critical" ? "#F7C5E6" : "#F5D66E", flexShrink: 0, marginTop: 4 }} />
                {a.message}
              </div>
            )) : <div style={{ fontSize: 12, color: "var(--text-secondary)" }}>All clear. No alerts.</div>}
          </Card>
          ...
          <Card shadow>
            ...
            {calendarEvents.length > 0 ? calendarEvents.slice(0, 3).map((ev) => (
              <div key={ev.event_id} style={{ display: "flex", gap: 6, alignItems: "center", padding: "6px 0", borderBottom: "1px solid var(--border-default)", fontSize: 12, color: "var(--text-secondary)" }}>
                <Calendar size={10} />
            ...
            )) : <div style={{ fontSize: 12, color: "var(--text-secondary)" }}>No events today.</div>}
          </Card>
        </div>
      </div>
    </div>
