    ...
    <div style={{ display: "flex", flexDirection: "column", height: "100%", overflow: "hidden" }}>
      ...
      <main style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden", padding: 0 }}>
        <div style={{ padding: "20px 28px 16px", borderBottom: "1px solid #D9D9D9" }}>
          ...
          <p style={{ color: "var(--text-secondary)", fontSize: 12, marginTop: 4 }}>Ask anything about your tasks, priorities, or sources</p>
        </div>
        ...
        <div style={{ flex: 1, overflow: "auto", padding: "24px 28px", display: "flex", flexDirection: "column", gap: 16 }}>
          {messages.length === 0 && (
            <div style={{ textAlign: "center", color: "var(--text-secondary)", fontSize: 13, marginTop: 40 }}>
              <SparkleIcon size={20} color="var(--blue-primary)" />
              ...
              <div style={{ display: "flex", gap: 8, justifyContent: "center", marginTop: 12, flexWrap: "wrap" }}>
                {["What's my #1 priority?", "Any blockers?", "Summarize recent emails"].map(q => (
                  <button key={q} onClick={() => { setInput(q); }}
                    style={{ background: "none", border: "1px solid #D9D9D9", padding: "5px 12px", color: "var(--text-secondary)", fontSize: 12, cursor: "pointer", fontFamily: "'IBM Plex Mono', monospace" }}>
                    {q}
                  </button>
                ))}
              </div>
          ...
          {messages.map((msg, i) => (
            <div key={i} style={{ display: "flex", justifyContent: msg.role === "user" ? "flex-end" : "flex-start", gap: 10, alignItems: "flex-start" }}>
              ...
              <div style={{ maxWidth: msg.role === "user" ? 420 : 580, border: "1px solid #D9D9D9", padding: "12px 16px", background: msg.role === "ai" ? "#FFFFFF" : "#F8F8F6" }}>
                <span style={{ color: "var(--text-primary)", fontSize: 13, lineHeight: 1.5 }}>{msg.content}</span>
                {msg.citations && msg.citations.length > 0 && (
                  <div style={{ marginTop: 8, color: "var(--text-secondary)", fontSize: 11, fontFamily: "'IBM Plex Mono', monospace" }}>
          ...
          {loading && (
            <div style={{ display: "flex", justifyContent: "flex-start", gap: 10 }}>
              ...
              <div style={{ border: "1px solid #D9D9D9", padding: "12px 16px", background: "var(--bg-elevated)" }}>
                <span style={{ color: "var(--text-secondary)", fontSize: 13, fontFamily: "'IBM Plex Mono', monospace" }}>Thinking...</span>
              </div>
            </div>
          )}
        </div>
        ...
        <div style={{ padding: "0 28px 24px" }}>
          <div style={{ display: "flex", alignItems: "center", border: "1px solid #D9D9D9", padding: "10px 14px", gap: 10, background: "var(--bg-elevated)" }}>
            <input value={input} onChange={e => setInput(e.target.value)}
              onKeyDown={e => e.key === "Enter" && handleSend()}
              placeholder="Ask about your tasks, priorities, or any source…"
              style={{ flex: 1, background: "none", border: "none", outline: "none", color: "var(--text-primary)", fontSize: 13, fontFamily: "'Inter', sans-serif" }} />
            <button onClick={handleSend}
              style={{ background: "var(--text-primary)", border: "none", width: 32, height: 32, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", flexShrink: 0 }}>
