...
export function A2AStatus() {
  ...
  if (loading) return <div className="flex items-center gap-2 px-3 py-1.5 text-xs text-[var(--text-secondary)]"><Loader2 className="w-3 h-3 animate-spin" /> A2A...</div>;
  ...
  return (
    <div className={`flex items-center gap-2 px-3 py-1.5 rounded-xl border text-xs ${isConnected || status?.status === "standalone" ? "border-[#BFD78D] bg-[#BFD78D]/10" : "border-[#F7C5E6] bg-[#F7C5E6]/10"}`}>
      <Network className={`w-3 h-3 ${isConnected ? "text-[#BFD78D]" : "text-[var(--text-secondary)]"}`} />
