...
import { dailyPlan as mockDailyPlan, planChanges as mockPlanChanges } from '../../data';
...
function mapApiTimeBlocks(apiBlocks: any[]): DailyPlanBlock[] {
  return apiBlocks.map((block: any, idx: number) => {
    const startHour = new Date(block.start).getHours();
    const period: 'morning' | 'afternoon' | 'evening' = startHour < 12 ? 'morning' : (startHour < 16 ? 'afternoon' : 'evening');
    const startTimeStr = new Date(block.start).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
    const endTimeStr = new Date(block.end).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
    
    return {
      id: block.task_id || `block-${idx}`,
      period,
      timeRange: `${startTimeStr} – ${endTimeStr}`,
      title: block.title || (block.slot_type === 'deep_work' ? 'Deep Work' : 'Collaboration Block'),
      description: block.slot_type === 'deep_work' ? 'Focus block for core strategic deliverables.' : 'Administrative reviews and alignment.',
      effort: `${Math.round(((new Date(block.end).getTime() - new Date(block.start).getTime()) / 3600000) * 10) / 10}h`,
      impact: block.priority === 'P0' || block.priority === 'P1' ? 'High' : 'Medium',
      tasks: [block.title],
      priority: block.priority || 'P2',
      aiRationale: `Scheduled based on ${block.slot_type} optimization rules. Cognitive load calculated at level ${Math.round(block.score / 10)}.`,
      dependencies: [],
    };
  });
}
...
export const DailyPlan: React.FC = () => {
  ...
  const [dailyPlan, setDailyPlan] = useState<DailyPlanBlock[]>(mockDailyPlan);
  const [planChanges, setPlanChanges] = useState<any[]>(mockPlanChanges);
  ...
  const loadPlanData = useCallback(async () => {
    try {
      ...
      // Map time blocks
      if (plan.time_blocked_plan && plan.time_blocked_plan.length > 0) {
        setDailyPlan(mapApiTimeBlocks(plan.time_blocked_plan));
      } else {
        // Fallback to top_priorities mapped as daily plan blocks
        const topTasks = plan.top_priorities || [];
        const mapped = topTasks.map((t, idx) => ({
          id: t.id,
          period: idx === 0 ? 'morning' as const : (idx === 1 ? 'afternoon' as const : 'evening' as const),
          timeRange: idx === 0 ? '08:00 – 11:00' : (idx === 1 ? '13:00 – 15:00' : '16:00 – 17:30'),
          title: t.title,
          description: t.description || 'Action item extracted from project stream.',
          effort: '2.5h',
          impact: t.priority === 'P0' || t.priority === 'P1' ? 'High' as const : 'Medium' as const,
          tasks: [t.title],
          priority: t.priority || 'P2',
          aiRationale: t.rationale || 'Prioritized based on compound latency metrics.',
      ...
      // Map plan changes alert
      if (plan.alerts && plan.alerts.length > 0) {
        const changes = plan.alerts.slice(0, 3).map((al, idx) => ({
          id: `change-${idx}`,
      ...
      console.warn("Failed to load real daily plan, falling back to mock plan: ", e);
    }
  }, []);
  ...
  const handleReoptimize = async () => {
    setLoading(true);
    try {
      const plan = await api.refreshPlan();
      if (plan) {
        await loadPlanData();
      }
    } catch (e) {
      console.error("Reoptimization failed: ", e);
    } finally {
      setLoading(false);
    }
  };
      ...
      >
        ...
        <div className="px-5 py-3 flex items-center gap-3 border-b border-[var(--border)]" style={{ background: 'rgba(132,163,147,0.04)' }}>
          ...
          <p className="text-xs text-[var(--text-primary)] font-medium">
            Active task queue reprioritized with live pipeline feedback
          </p>
      ...
      <div className="flex items-end justify-between">
        ...
        <div className="flex items-center gap-3">
          ...
          <button 
            ...
            className="flex items-center gap-1.5 px-3 py-2 rounded-lg border border-[var(--success)]/20 bg-[var(--surface)] text-[var(--success)] text-xs font-semibold hover:bg-[var(--success)]/8 hover:border-[var(--success)]/35 transition-colors cursor-pointer"
          >
